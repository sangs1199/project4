...
public class MarksheetModel {
    ...
    public List search(MarksheetBean bean, int pageNo, int pageSize)
        ...
        if (bean != null) {
            ...
            if (bean.getStudentId()> 0) {
            	System.out.println("service" + bean.getId());
    ...
    public List getMeritList(int pageNo, int pageSize)
        ...
        ArrayList list = new ArrayList();
        StringBuffer sql = new StringBuffer(
                "SELECT `ID`,`ROLL_NO`, `NAME`, `PHYSICS`, `CHEMISTRY`, `MATHS` , (PHYSICS + CHEMISTRY + MATHS) AS total FROM `ST_MARKSHEET` WHERE (PHYSICS>=34 && CHEMISTRY>=34 && MATHS>=34) ORDER BY total DESC");
        // if page size is greater than zero then apply pagination
        if (pageSize > 0) {
            // Calculate start record index
            pageNo = (pageNo - 1) * pageSize;
